...
{
    if (!fBins) {
        Info("", "TH2PolyLite::InsertBin");
        fBins = new std::vector<TH2PolyBinLite*>();
		//fBins->SetOwner();
	}
//	fNcells++;
    Int_t ibin = fNcells++ - kNOverflow;
    TH2PolyBinLite *bin = new TH2PolyBinLite(n, x, y, ibin);
...
Int_t TH2PolyLite::FindBin(Double_t x, Double_t y, Double_t)
{
    ...
    {
        Info("DRAW", "TH2PolyLite::FindBin::EmptyCell");
        return -5;
    }
    ...
    Info("DRAW", (std::string("TH2PolyLite::FindBin::CellNum: ") + std::to_string(n) + " x " + std::to_string(m) + " -> " + std::to_string(n + fCellX*m)).c_str());

	for (auto bin : fCells[n + fCellX*m])
	{
        Info("DRAW", (std::string("Looking inside bin: ") + std::to_string(bin->GetBinNumber())).c_str());
		if (bin->IsInside(x, y)) return bin->GetBinNumber();
	}

	// If the search has not returned a bin, the point must be on "the sea"
    Info("DRAW", "TH2PolyLite::FindBin::Cell does not have bin in that place");
	return -5;
}
...
{
    if (fNcells <= kNOverflow || !fBins->size()) return 0;
                             ...
                             Double_t ylow, Double_t yup, Int_t n, Int_t m)
{
    ...
    for (i = 0; i < kNOverflow; i++) fOverflow[i] = 0.;
...
{
    if (bin > GetNumberOfBins() || bin == 0 || bin < -kNOverflow) return;
	if (bin > 0)
...
Bool_t TH2PolyLite::IsInsideBin(Int_t binnr, Double_t x, Double_t y)
{
   if (!fBins) return false;
   TH2PolyBinLite* bin = fBins->at(binnr);
   if (!bin) return false;
   return bin->IsInside(x,y);
}

void TH2PolyLite::GetStats(Double_t *stats) const
{
   stats[0] = fTsumw;
   stats[1] = fTsumw2;
   stats[2] = fTsumwx;
