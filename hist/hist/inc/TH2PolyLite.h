...
public:
    ...
    //virtual Int_t         Fill(const char* name, Double_t w);						// IS THIS NEEDED TO BE SET??
    virtual Int_t   Fill(Int_t id, Double_t w);
    ...
    std::vector<TH2PolyBinLite*>       *GetBins() const { return fBins; }                                // Returns the TList of all bins in the histogram
...
protected:
    ...
    std::vector<TH2PolyBinLite*>   *fBins{nullptr};              //List of bins. The list owns the contained objects
    ...
    std::vector<TH2PolyBinLite*>   *fCells{nullptr};             //[fNCells] The array of TLists that store the bins that intersect with each cell. List do not own the contained objects
