...
public:
    ...
    TH2PolyBinLite(Int_t nVerts, const Double_t* _x, const Double_t* _y, Int_t bin_number);
	virtual ~TH2PolyBinLite();

	void      ClearContent() { fContent = 0; }
    void      Fill(Double_t w) { fContent = fContent + w; SetChanged(kTRUE); }
    ...
    Int_t     BuildFullPolyDescription(Double_t *_x, Double_t *_y);

    Bool_t    IsInside(Double_t _x, Double_t _y);
	void      SetChanged(Bool_t flag) { fChanged = flag; }
    void      SetContent(Double_t content) { fContent = content; SetChanged(kTRUE); }
